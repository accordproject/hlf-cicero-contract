 ...
 * @requires {Promise<*[]>} an array of templates
 */
async function loadTemplates(ctx, contractText, initClauses) {
    ...
    if(clauses.length > 0) {
        ...
        for( let n=0; n < clauses.length; n++ ) {
            ...
            templates[clauseNode.clauseid] = template;
...
class CiceroContract extends Contract {
    ...
    async ensureNotInitialized(ctx) {
        const markdown = await ctx.stub.getState('Markdown');
     ...
     */
    async upgrade(ctx) {
        const markdown = await this.ensureInitialized(ctx);
    ...
    async initialize(ctx, contractText) {
        ...
        await this.ensureNotInitialized(ctx);
        ...
        return `Initialized ${this.templates.length} clauses.`;
    }

    /**
     * Triggers a specified clause within the contract, passing a request JSON as input.
     * @param {*} ctx the HLF context
     * @param {*} clauseId the ID of a clause present within the markdown text passed to initialize
    ...
    async trigger(ctx, clauseId, requestText) {
        ...
        // set the clause data
        const template = this.templates[clauseId];
        if(!template) {
            throw new Error(`ERROR: no template found for clause id: ${clauseId}`);
        }
        // @ts-ignore
        const clause = new Clause(template);
        // @ts-ignore
