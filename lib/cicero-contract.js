...
async function loadTemplates(ctx, contractText, initClauses) {
    ...
    if(clauses.length > 0) {
        ...
        for( let n=0; n < clauses.length; n++ ) {
            ...
            clauseData.clauseId = clauseNode.clauseid; // ensure consistency across peers
    ...
    async trigger(ctx, clauseId, requestText) {
        ...
        if (result.emit.length > 0) {
            result.emit.forEach( (event,index) => {
                ...
                ctx.stub.setEvent(`${request.transactionId}-Events-${index}`, Buffer.from(JSON.stringify(event)));
            });
        }
